...
import com.sidziuk.GameRulesAlgebra.numberCardsOnHands
...
case class OHellGame(deck: Deck,
                     players: Seq[Player],
                     winner: Option[Seq[Player]] = None,
                     desk: Desk = Desk(),
                     numberCardsOnHands: Int = numberCardsOnHands,
                     scoreHistory: Map[String, Seq[Int]],
...
trait GameRulesAlgebra {
  def createNewGame(players: Seq[Player]): OHellGame
...
object GameRulesAlgebra {
  ...
  val flipRoundNumber = 3
  val maxRounds = 5


  implicit def gameRulesAlgebraSync(implicit deckalg: DeckAlgebra, playeralg: PlayerAlgebra): GameRulesAlgebra = new GameRulesAlgebra {

    def nexPlayerIndex(currentPlayerIndex: Int, players: Seq[Player]) = (currentPlayerIndex + 1) % players.size

    override def createNewGame(players: Seq[Player]): OHellGame = {


      @tailrec
      def cardgg(players: Seq[Player], deck: Deck, cardNumber: Int): (Seq[Player], Deck) = {
      ...
      val dealerId = randomPlayer.id
      val movePlayerId = players(nexPlayerIndex(players.indexOf(randomPlayer), players)).id
      val (newplayers, deck): (Seq[Player], Deck) = cardgg(players, deckalg.createShuffledDesk, numberCardsOnHands)
      val superPlayers = newplayers.map { player =>
        val mayBeDealerPlayer = if (player.id == dealerId) player.copy(isDealer = true) else player
        if (player.id == movePlayerId) mayBeDealerPlayer.copy(isHaveMove = true, possibleBids = Option(0 to numberCardsOnHands)) else mayBeDealerPlayer
      }
      val deckWithTrump = deckalg.getTrump(deck)
      val newMap: Map[String, Seq[Int]] = Map()
      val mapscore: Map[String, Seq[Int]] = players.foldLeft(newMap) { case (map, pl) => map + (pl.id -> Seq()) }
      OHellGame(deckWithTrump, superPlayers, scoreHistory = mapscore)
    }

    def checkIfMoveValid(game: OHellGame, move: Move): Boolean =
      if (game.players.iterator.filter(_.isHaveMove).map(_.id).contains(move.playerId)
        ...
        move match {
          case Move(playerId, moveType, card, bid)
            if moveType == Bid &&
              card.isEmpty &&
              bid.nonEmpty &&
              game.players.filter(_.id == playerId).head.possibleBids.get.contains(bid.get) => true
          case Move(playerId, moveType, card, bid)
            if moveType == PlayCard &&
              card.nonEmpty &&
              bid.isEmpty &&
              game.players.filter(_.id == playerId).head.hand.get.contains(card.get) && {
              game.desk.firstPlayer match {
                case Some(value) => card.get.suit == value._2 ||
                  (card.get.suit != value._2 && game.players.filter(_.id == playerId).head.hand.get.exists(_.suit != value._2))
                case None => true
              }
            } => true
          case _ => false
      ...
      if (!game.ifGameEnd && checkIfMoveValid(game, move)) {
        move match {
          case Move(playerId, moveType, _, bid) if moveType == Bid =>
            ...
            val currentPlayer = players.filter(_.id == playerId).head
                ...
                players.zipWithIndex.map { case (player, index) =>
                  if (player.id == playerId) {
                    ...
                    if (player.isDealer) {
                      ...
                      player.copy(
                        possibleBids = Option((0 to game.numberCardsOnHands)
                          .filter { number =>
          ...
          case Move(playerId, moveType, card, _) if moveType == PlayCard =>
            ...
            val newCardsOnDesk = game.desk.cards match {
              ...
              case None => Desk(Option(Map(playerId -> card.get)), Option(currentPlayer.id, card.get.suit))
            }
            if (newCardsOnDesk.cards.get.size == players.size) {

              def getWinnerFromCardsOnDesk(cardsOnDesk: Desk, trumpSuit: Suit): String = {
              ...
              val deskWinner = getWinnerFromCardsOnDesk(newCardsOnDesk, game.deck.trump.get.suit)
              //              println(deskWinner)

              val newPlayers = players.map { pl =>
                val pl1 = if (pl.id == deskWinner) {
                  pl.copy(tricksNumber = addTrick(pl.tricksNumber))
                } else pl
                if (pl1.id == playerId) {
                  playeralg.playCardFromPlayerHand(pl1, card.get)._1.copy(isHaveMove = pl.id == deskWinner)
                }
                else
                  pl1.copy(isHaveMove = pl1.id == deskWinner)
              }
              if (newPlayers.forall(_.hand.isEmpty)) {

                @tailrec
                def cardgg(players: Seq[Player], deck: Deck, cardNumberOnHands: Int): (Seq[Player], Deck) = {
                ...
                val newScoreHistory = game.scoreHistory.map { case (key, value) =>
                  ...
                  val sc1 = newPlayersWithScore.filter(_.id == key).head.score
                ...
                if (game.currentGameRound >= maxRounds) {
                  ...
                  val winners: Option[Seq[Player]] = Option(newPlayersWithScore.filter(_ == maxWinner))

                  game.copy(players = newPlayersWithScore, winner = winners, ifGameEnd = true, desk = Desk(), scoreHistory = newScoreHistory)

                } else {
                  val newCardNumberOnHands = if (game.currentGameRound < flipRoundNumber) game.numberCardsOnHands - 1 else game.numberCardsOnHands + 1
                  ...
                  val (newPlayersWithCards, newDekWithTrump2) = cardgg(newPlayersWithScore, newDekWithTrump, newCardNumberOnHands)
                  ...
                  val newPlayers2 = newPlayersWithCards.zipWithIndex.map { case (pl, index) =>
                    ...
                    if (index == nexPlayerIndex(currentDealerIndex, newPlayersWithCards)) {
                      //                    println("--------------")
                      pl.copy(tricksNumber = None, bid = None, possibleBids = None, isDealer = true, isHaveMove = false)
                    }
                    else if (index == nexPlayerIndex(nexPlayerIndex(currentDealerIndex, newPlayersWithCards), newPlayersWithCards)) {
                      pl.copy(tricksNumber = None, bid = None, possibleBids = Option(0 to newCardNumberOnHands), isDealer = false, isHaveMove = true)
                    }
                    else {
                      pl.copy(tricksNumber = None, bid = None, possibleBids = None, isDealer = false, isHaveMove = false)
                    }
                  }

                  OHellGame(
                    deck = newDekWithTrump2,
                    players = newPlayers2,
...
object d extends App {
  ...
  val oo = GameRulesAlgebra.gameRulesAlgebraSync.createNewGame(Seq(Player("1", "Vasa"), Player("2", "Hoha"), Player("3", "Vasa")))
  println(oo.players)
  println(oo.moveType)
  ...
  println("kozer " + ll3.deck.trump.get.suit)
  ...
  println("kozer " + llq3.deck.trump.get.suit)
  ...
  println("kozer " + bid3.deck.trump.get.suit)
  ...
  println("kozer " + bid6.deck.trump.get.suit)
  ...
  println("kozer " + bid33.deck.trump.get.suit)
